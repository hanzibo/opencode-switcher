...
> **关键词**：`动态模板` `复合语法` `转义字符` `正则表达式` `GTK对话框` `数据清洗`
...
- **测试用例驱动设计**：在 `scratch` 临时工作目录中建立了完备的测试脚本 `test_placeholder_composite.py`，覆盖了纯提示符、纯默认值、复合冒号/等号转义、空格保留、空默认值等 10 余个边界场景，极大地防范了代码发布时的回归风险。
...
- **最终方案**：
  ...
  ```
- **预防建议**：当定义含有转义约定的自定义微型语法结构时，解析出的内容在面向最终用户展示（UI、控件、剪切板）时必须通过独立的清理映射（Unescape / Decoupling Cleaner）解除转义，以实现“内部处理”与“界面展示”的隔离。
...
- **基于正则负向后瞻的条件拆解**：
  - 正则表达式中 `(?<!\\)` 能够在词法匹配时有效拦截转义修饰过的特定字符（例如只匹配未转义的 `=` 作为属性边界）。
