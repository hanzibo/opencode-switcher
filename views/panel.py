...
class SearchPanel:
    ...
    def __init__(self):
        ...
        self._ai_copy_in_progress = False
        self._show_time = 0.0
        self._active_tab = 0
    ...
    def _set_theme(self, name: str):
        ...
        css = (
            ...
            ".row { padding: 12px 18px; border-radius: 12px; margin: 4px 10px; border-left: 4px solid transparent; }"
            ...
            ".custom-dialog notebook, .custom-dialog notebook > stack, .custom-dialog notebook > header, "
            ".custom-dialog notebook tabs, .custom-dialog notebook tab, "
            ".custom-dialog viewport.frame, .custom-dialog scrolledwindow.frame, "
            ".custom-dialog scrolledwindow > border, .custom-dialog viewport > border, "
            ".custom-dialog scrolledwindow box, .custom-dialog viewport box, "
            ".custom-dialog notebook box, .custom-dialog notebook stack box { "
            ...
            "filechooser treeview, filechooser treeview.view { "
            "background-color: %(input_bg)s; color: %(text_fg)s; border: none; }"
            "filechooser treeview.view:hover { background-color: %(hover_bg)s; }"
            "filechooser treeview.view:selected { background-color: %(sel_bg)s; color: %(text_fg)s; }"
            "filechooser placesview, filechooser placesview list, filechooser .sidebar { "
            "background-color: %(dialog_bg)s; color: %(text_fg)s; border: none; }"
            "filechooser placesview row, filechooser .sidebar row { "
            "background-color: transparent; color: %(text_fg)s; }"
            "filechooser placesview row:hover, filechooser .sidebar row:hover { "
            "background-color: %(hover_bg)s; }"
            "filechooser placesview row:selected, filechooser .sidebar row:selected { "
            "background-color: %(sel_bg)s; color: %(text_fg)s; }"
            "filechooser button { "
            "background-color: %(btn_bg)s; color: %(text_fg)s; border: 1px solid %(btn_border)s; }"
            "filechooser button:hover { background-color: %(btn_hover)s; }"
            "filechooser button:active { background-color: %(btn_active)s; }"
            "filechooser entry { "
            "background-color: %(input_bg)s; color: %(input_fg)s; border: 1px solid %(input_border)s; }"
            "filechooser label { color: %(text_fg)s; }"
            "filechooser paned, filechooser separator { background-color: %(dialog_bg)s; }"
            "filechooser treeview header button, filechooser button.header { "
            "background-color: %(btn_bg)s; color: %(text_fg)s; border: 1px solid %(btn_border)s; }"
            "filechooser treeview header button:hover, filechooser button.header:hover { "
    ...
    def set_clipboard_panel(self, panel: ClipboardPanel, clip_store, cat_store=None):
        ...
        panel.on_ai_copy_started = self._on_ai_copy_started
        panel.on_ai_copy_finished = self._on_ai_copy_finished
    ...
    def _on_ai_copy_started(self):
        self._ai_copy_in_progress = True

    def _on_ai_copy_finished(self):
        GLib.timeout_add(300, self._clear_ai_copy_flag)

    def _clear_ai_copy_flag(self):
        self._ai_copy_in_progress = False
        return False
    ...
    def _on_focus_out(self, *_args):
        ...
        def do_hide():
            if (self._menu_active or self._delete_in_progress or 
                    self._dialog_active or getattr(self, "_combo_popup_active", False) or
                    self._ai_copy_in_progress):
                return False
            # 同步兜底检查下拉框是否正处于弹出状态
