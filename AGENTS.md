...
Linux GTK3 desktop tray app for switching between OpenCode (CLI) sessions, clipboard history management, and an AI assistant sidebar. Python 3 + GTK3 + AyatanaAppIndicator. **No CI/linter/formatter/typechecker. No automated tests.**
...
**Commit convention**: `fix(area):`, `feat(area):`, `improve(area):`, `refactor(area):`, `optimize(area):`, `perf(area):`, `style(area):`, `docs(area):`, `merge:`. Area prefix follows module (e.g., `ai-panel`, `theme`, `tool-registry`, `clipboard`). ~418 commits, all by single author.
...
| `clipboard_panel.py` | 2161 | Clipboard panel — assembles subcomponents + event routing |
| `clipboard_store.py` | 1072 | God module — 12 classes: classification, clipboard storage, categories, conversations, LLM settings, prompts |
| `ai_chat_panel.py` | 2906 | AI assistant — WebView, LLM dialog, ReAct tool calls, round nav, lightbox, subagent status |
| `ai_tool_loop.py` | 238 | ReAct loop (max 25 iterations) |
| `ai_html_template.py` | 1183 | WebView HTML template + KaTeX + lightbox JS |
...
| `tool_registry/` | 4930 | 25 tools across 12 modules |
...
- **`add_provider_for_screen`** in 3 files — leaks CSS globally (accepted tradeoff).
