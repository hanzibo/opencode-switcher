"""Heuristic code/text classification and programming language detection.

Extracted from stores/clipboard_store.py to decouple pure text analysis
algorithms from data storage and persistence layers.
"""
...
HTML_START_RE = re.compile(
    r"^\s*<(html|head|body|div|span|p|a|ul|ol|li|table|tr|td|script|style|link|meta|xml)\b",
    re.IGNORECASE,
)
HTML_END_RE = re.compile(r"</[a-zA-Z][a-zA-Z0-9]*>")
SHEBANG_RE = re.compile(r"^#!\s*/(bin|usr)/")
CLI_CMD_RE = re.compile(
    r"^\s*(sudo\s+)?(apt-get|yum|docker|systemctl|pip install|npm install|git clone|yarn add|pnpm add|chmod\s+\+x)\b"
)
...
TYPED_DECL_RE = re.compile(
    r"\b(public|private|protected)\s+(class|interface|void|int|double|float|char|bool|boolean|string)\b"
)
...
GENERIC_KW_RE = re.compile(
    r"\b(interface|void|printf|println|nullptr|undefined|typeof|elseif|elif|sizeof|struct|enum|static|class)\b"
)
...
def classify_text(text: str) -> str:
    """Classify plain text into \"text\", \"link\", or \"code\"."""
    stripped = text.strip()
    if not stripped:
        return "text"
...
def detect_language_name(text: str) -> Optional[str]:
    """Detect programming language name from snippet using heuristic scoring."""
    stripped = text.strip()
    if not stripped:
        return None
